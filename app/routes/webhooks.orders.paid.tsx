 ...
 * Tracks when orders are successfully paid - important for delivery confirmations
 */
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    console.log("Processing order payment webhook for shop:", shop);
    ...
    if (!phone && !email) {
      console.log("No customer identifiers found in paid order, skipping risk tracking");
      return json({ status: "ok" });
    }

    console.log("Processing paid order for customer:", { 
      hasPhone: !!phone, 
      hasEmail: !!email, 
      hasAddress: !!address,
      ...
      paymentGateways: payment_gateway_names
    });
    ...
    try {
      ...
      // Record the payment event - this indicates successful payment completion
      ...
      console.log(`✓ Order payment tracked for customer profile: ${customerProfile.id} (amount: ${total_price} ${currency})`);
    } catch (dbError) {
      console.error("Database error processing paid order:", dbError);
